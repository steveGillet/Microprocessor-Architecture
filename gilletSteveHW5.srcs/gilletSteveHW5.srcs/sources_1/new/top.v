...
module top(
    ...
    initial begin
        {braReg, pcReg, pc1, a, b, rAA, dataIn, nXORv, dataOutReg, fReg, iRreg, dData, address, vReg, cReg, nReg, zReg, mWreg, pSreg, rWreg, aAreg, bAreg, fSreg, sH, sHex, dAreg, mDreg} = 0;
    end

    muxC muxC(.reset(reset), .pCin(pcReg), .brA(brA), .rAA(rAA), .bS(bSreg), .pS(pSreg), .z(zReg), .pCout(pCoutMuxC));
